/*
 * drivers/cpufreq/cpufreq_impulsex.c
 *
...
static bool mdss_screen_on;

struct cpufreq_impulsex_cpuinfo {
...
static DEFINE_PER_CPU(struct cpufreq_impulsex_cpuinfo, cpuinfo);
...
/* Maximun freq. when screen is off. */
...
static void cpufreq_impulsex_timer_resched(unsigned long cpu)
{
	struct cpufreq_impulsex_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
	u64 expires;
	unsigned long flags;
 ...
 * function.
 */
static void cpufreq_impulsex_timer_start(int cpu)
{
	struct cpufreq_impulsex_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
...
static unsigned int choose_freq(
	struct cpufreq_impulsex_cpuinfo *pcpu, unsigned int loadadjfreq)
{
...
static void cpufreq_impulsex_timer(unsigned long data)
{
	u64 now;
	unsigned int delta_time;
	u64 cputime_speedadj;
	int cpu_load;
	struct cpufreq_impulsex_cpuinfo *pcpu =
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_impulsex_timer_resched(data);
...
static void cpufreq_impulsex_idle_end(void)
{
	struct cpufreq_impulsex_cpuinfo *pcpu =
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		cpufreq_impulsex_timer_resched(smp_processor_id());
	} else if (time_after_eq(jiffies, pcpu->cpu_timer.expires)) {
		del_timer(&pcpu->cpu_timer);
		del_timer(&pcpu->cpu_slack_timer);
		cpufreq_impulsex_timer(smp_processor_id());
	}

	up_read(&pcpu->enable_sem);
}

static int cpufreq_impulsex_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			struct cpufreq_impulsex_cpuinfo *pjcpu;
			u64 hvt = 0;
...
static void cpufreq_impulsex_boost(void)
{
	int i;
	int anyboost = 0;
...
static int cpufreq_impulsex_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_impulsex_notifier,
};
...
static struct attribute *impulsex_attributes[] = {
...
static struct attribute_group impulsex_attr_group = {
	.attrs = impulsex_attributes,
	.name = "impulsex",
};

static int cpufreq_impulsex_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
	if (val == IDLE_END)
		cpufreq_impulsex_idle_end();

	return 0;
}

static struct notifier_block cpufreq_impulsex_idle_nb = {
	.notifier_call = cpufreq_impulsex_idle_notifier,
};

static int cpufreq_governor_impulsex(struct cpufreq_policy *policy,
		unsigned int event)
{
	int rc;
	unsigned int j;
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			pcpu->last_evaluated_jiffy = get_jiffies_64();
			if (cpu_online(j))
				cpufreq_impulsex_timer_start(j);
		...
		idle_notifier_register(&cpufreq_impulsex_idle_nb);
	...
	case CPUFREQ_GOV_STOP:
		...
		cpufreq_unregister_notifier(
			&cpufreq_notifier_block, CPUFREQ_TRANSITION_NOTIFIER);
		idle_notifier_unregister(&cpufreq_impulsex_idle_nb);
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_impulsex
static
#endif
struct cpufreq_governor cpufreq_gov_impulsex = {
	.name = "impulsex",
	.governor = cpufreq_governor_impulsex,
	.max_transition_latency = 10000000,
	.owner = THIS_MODULE,
};

static void cpufreq_impulsex_nop_timer(unsigned long data)
{
}

static int __init cpufreq_impulsex_init(void)
{
	unsigned int i;
	struct cpufreq_impulsex_cpuinfo *pcpu;
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_impulsex_timer;
		pcpu->cpu_timer.data = i;
		init_timer(&pcpu->cpu_slack_timer);
		pcpu->cpu_slack_timer.function = cpufreq_impulsex_nop_timer;
	...
	mutex_init(&gov_lock);
	speedchange_task =
		kthread_create(cpufreq_impulsex_speedchange_task, NULL,
			       "cfimpulsex");
	...
	return cpufreq_register_governor(&cpufreq_gov_impulsex);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_IMPULSEX
fs_initcall(cpufreq_impulsex_init);
#else
module_init(cpufreq_impulsex_init);
#endif

static void __exit cpufreq_impulsex_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_impulsex);
	kthread_stop(speedchange_task);
	put_task_struct(speedchange_task);
}

module_exit(cpufreq_impulsex_exit);

MODULE_AUTHOR("Mike Chan <mike@android.com> && modify by xperiafan13 <mundorap2010@gmail.com>");
MODULE_DESCRIPTION("'cpufreq_impulsex' - A cpufreq governor for "
	"all purpose.");
MODULE_LICENSE("GPL");
